...
const ActivitiesCardBody = ({
  item = null,
  childActivityEventType = null,
  setIsShowPaymentComp,
  isShowPaymentComp,
  navigation,
}) => {
  // console.log('item', item);
  //console.log('childActivityEventType', childActivityEventType);
  let event = {};
  ...
  const {axiosConfig, countryTimestamp, countryDateTime} =
  ...
  switch (childActivityEventType) {
    case 'learningSession':
      ...
      // console.log('item.learning_session', item.learning_session);
      break;
    ...
    case 'meetup':
      ...
      event = item.meetup;
      fee = item?.meetup?.fee;
      eventRegistration = item?.meetup_registration;
      break;
    ...
    case 'auction':
      ...
      console.log('auction', item.auction);
      break;
  }
  ...
  const handleJoinNow = () => {
    console.log('room', event?.room_id);
    ...
    if (childActivityEventType == 'liveChat') {
      navigation.navigate('VideoSdk', {
        ...
        type: 'videoChat',
        starName: event?.star?.first_name + ' ' + event?.star?.last_name,
      });
      ...
      // return console.log("fdhfshf____>", event)

      // return alert(event?.room_id)
  ...
  const downlodeTicket = () => {
    const data = {
      id: event?.id,
      uid: eventRegistration?.user_id,
    };
    console.log(data);
    console.log(AppUrl.DownlodMeetUpTicket);
    Toast.show('Please wait downloading...', Toast.durations.SHORT);

    axios
      .post(AppUrl.DownlodMeetUpTicket, data, axiosConfig)
      .then(res => {
  ...
  let event_start = new Date(countryDateTime(event_start_time, '')).getTime();
  let event_end = new Date(countryDateTime(event_end_time, '')).getTime();

  let EventDateWithEndTime = new Date(
    moment(countryDateTime(ActualEventDate + ' ' + EndTime, '')),
  );
  let EventDateWithStartTime = new Date(
    moment(countryDateTime(ActualEventDate + ' ' + StartTime, '')),
  );

  let CurrentDateWithTime = new Date();

  useEffect(() => {
    event_start = new Date(countryDateTime(event_start_time, '')).getTime();
    event_end = new Date(countryDateTime(event_end_time, '')).getTime();

    EventDateWithEndTime = new Date(
      moment(countryDateTime(ActualEventDate + ' ' + EndTime, '')),
    );
    EventDateWithStartTime = new Date(
      moment(countryDateTime(ActualEventDate + ' ' + StartTime, '')),
    );
  ...
  const timeLeftForEvent = () => {
    let CurrentDateWithTime = new Date();
    let EventDateWithStartTime = new Date(
      moment(countryDateTime(ActualEventDate + ' ' + StartTime, '')),
    );
    let time_left = (EventDateWithStartTime - CurrentDateWithTime) / 1000;
      ...
      <View style={{flexDirection: 'row'}}>
        <View style={styles.Container}>
          {event?.banner == null && eventType == '' ? (
            ...
            <>
              <TouchableOpacity onPress={showSouvinorDetails}>
                <Image
                  source={{
                    uri: `${AppUrl.imageCdn + event?.image}`,
                  }}
                  style={styles.ImgBanner}
                />
            ...
            <>
              <TouchableOpacity
                ...
                <Image
                  source={{
                    uri: `${AppUrl.imageCdn + event?.product_image}`,
                  }}
                  style={styles.ImgBanner}
                />
            ...
            <>
              <Image
                source={
                  event?.banner
                    ? {
                        uri: `${AppUrl.imageCdn + event?.banner}`,
                      }
                      ...
                      <>
                        ...
                        {eventStart && (
                          <View
                            ...
                            <CountDown
                              until={timeLeftForEvent()}
                              onFinish={() => setEventStart(false)}
                              // onPress={() => alert('hello')}
                              timeToShow={['D', 'H', 'M', 'S']}
                              size={15}
                              digitStyle={{
                ...
                {eventType === 'qna' && (
                  <View
                    ...
                    event_end > new Date().getTime() ? (
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          Toast.show('Your Session expired', Toast.SHORT);
                        }}>
          ...
          <View style={styles.bannerTag}>
            <Image source={eventType === 'auction' ? null : imagePath.BgTag} />
          </View>
...
export default ActivitiesCardBody;
