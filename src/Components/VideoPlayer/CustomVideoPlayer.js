...
const CustomVideoPlayer = ({ videoUrl, postConfig, thumbnail }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      setIsLoading(true);
      if (postConfig.index === postConfig.currentIndex) {
        setIsPlaying(true);
      } else {
        setIsPlaying(false);
      }
    }, [postConfig.currentIndex]),
  );
  useEffect(() => {
    postConfig.index === postConfig.currentIndex && setVideoRender(true);
  });
  ...
  const handleMutePress = () => {
    if (postConfig.isMuted) {
      postConfig.setIsMuted(false);
      //setVolume(0);
    } else {
      postConfig.setIsMuted(true);
      //setVolume(1);
    }
  };
  return (
      ...
      <TouchableWithoutFeedback onPress={handleHideControl}>
        {videRender ? (
          <Video
            source={{ uri: videoUrl }}
            ref={videoPlayer}
            style={[styles.videoPlayer, { height: 250 }]}
            useTextureView={true}
